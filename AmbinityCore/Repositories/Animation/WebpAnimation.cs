...
public class WebpAnimation : ObservableObject, IAnimation
{
    ...
    public WebpAnimation(string name)
    {
        Name = name;
        UID = Guid.NewGuid();
    }
    ...
    {
        if (!File.Exists(Path.Combine(LocalPath, "animation.webp")))
        {
            //todo revert to default animation
            Log.Error("Animation file not found: " + Path.Combine(LocalPath, "animation.webp"));
            return;
        }
        AnimationPath = Path.Combine(LocalPath,"animation.webp");
