...
public class VideoAnimation : ObservableObject, IAnimation
{
    ...
    public VideoAnimation(string name)
    {
        Name = name;
        UID = Guid.NewGuid();
    }
    ...
    {
        if (!File.Exists(Path.Combine(LocalPath, "video.mp4")))
        {
            //todo revert to default animation
            Log.Error("Animation file not found: " + Path.Combine(LocalPath, "video.mp4"));
            return;
        }
        VideoPath = Path.Combine(LocalPath,"video.mp4");
    ...
    [JsonIgnore] public string VideoPath { get; set; }
}
