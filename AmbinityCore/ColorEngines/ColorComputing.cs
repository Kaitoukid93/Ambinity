...
public static class ColorComputing
{
    ...
    public static unsafe void PlotPixel(FrameBuffer _imageBuffer, int x, int y,
        byte rNew, byte gNew, byte bNew)
    {
        int offset = (_imageBuffer.FrameWidth * 4 * y) + (x * 4);

        // skip “transparent” black
        if (rNew == 0 && gNew == 0 && bNew == 0)
            return;
        ...
        fixed (byte* ptr = _imageBuffer.PixelData)
        {
            ...
            // Equal blend
            byte r = (byte)((rOld + rNew) >> 1);
            byte g = (byte)((gOld + gNew) >> 1);
            byte b = (byte)((bOld + bNew) >> 1);
