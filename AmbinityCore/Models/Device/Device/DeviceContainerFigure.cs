...
public class DeviceContainerFigure : ContainerFigure, IAssetSelectable
{
    ...
    public Rect? GetLinkedBoundingBox()
    {
        if (ChildItem == null || ChildItem.GroupID == Guid.Empty || Canvas == null)
            return null;

        var groupId = ChildItem.GroupID;

        // Find all DeviceContainerFigure on the same canvas with the same GroupID
        var linkedFigures = Canvas.Figures
            .OfType<DeviceContainerFigure>()
            .Where(f => f.ChildItem != null && f.ChildItem.GroupID == groupId && groupId != Guid.Empty)
            .ToList();

        if (linkedFigures.Count == 0)
            return null;

        // Calculate the bounding box
        float minX = linkedFigures.Min(f => f.X);
        float minY = linkedFigures.Min(f => f.Y);
        float maxX = linkedFigures.Max(f => f.X + f.Width);
        float maxY = linkedFigures.Max(f => f.Y + f.Height);
