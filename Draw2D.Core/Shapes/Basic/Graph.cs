          ...
          //  pen.Freeze();
            ...
            using (StreamGeometryContext ctx = geom.Open())
            {
                foreach (var edge in _graph.Edges)
                {
                    ...
                    var sourcePoint = new Avalonia.Point(arr[0],arr[1]);
                    arr = Canvas.CoordinateSystem.ToScreenSpace(edge.Target);
                    var targetPoint = new Avalonia.Point(arr[0], arr[1]);

                    ctx.BeginFigure(sourcePoint, false);
                    ctx.LineTo(targetPoint);

                   // geom.Freeze();
                    dc.DrawGeometry(null, pen, geom);
                }

                var strokeBrushNode = new ImmutableSolidColorBrush(StrokeColor);
              //  strokeBrushNode.Freeze();
                ...
                var fillBrushNode = new ImmutableSolidColorBrush(StrokeColor);
               // fillBrushNode.Freeze();
               // pen.Freeze();

                foreach (var vertex in _graph.Vertices.Select(Canvas.CoordinateSystem.ToScreenSpace))
                {
                    dc.DrawEllipse(fillBrushNode, penNode, new Avalonia.Point(vertex[0], vertex[1]), 2, 2);
                }
            }
